...
public class Sort{
    ...
    public static void quickSort(Student[] array, int low, int high, int Finals) {
        SortingStudentsByGPA sortingStudentsByGPA = new SortingStudentsByGPA(Finals);
        if (array.length != 0) {
            if (low < high) {
                ...
                if (low < j) {
                    quickSort(array, low, j, Finals);
                }

                if (high > i) {
                    quickSort(array, i, high, Finals);
                }

            }
        }
    }
    public static void MergeSort2(Student[] array, int left, int middle, int right,int isFinal){
        SortingStudentsByGPA sortingStudentsByGPA = new SortingStudentsByGPA(isFinal);
    ...
    public static void MergeSort1(Student[] array, int right, int left,int isFinal){
        if (left < right){
            int middle = (right + left) / 2;
            MergeSort1(array, middle, left, isFinal);//для дальнейшего разбиения левой части
            MergeSort1(array, right, middle + 1, isFinal);//для дальнейшего разбиения правой части
            MergeSort2(array, left, middle, right, isFinal);//слияние двух отсортированных частей
        }
    }
}